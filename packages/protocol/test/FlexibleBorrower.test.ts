...
import {
  ...
  deployFlexibleFundedTranchedPool,
...
describe("FlexibleBorrower", async () => {
  ...
  const setupTest = deployments.createFixture(async ({deployments}) => {
    ...
    const {tranchedPool, creditLine} = await deployFlexibleFundedTranchedPool({
      borrower,
  ...
  describe("drawdown", async () => {
    ...
    it("should block you from drawing down on some random credit line", async () => {
      ...
      const deployments = await deployFlexibleFundedTranchedPool({
        id: "TranchedPool",
        borrower,
        borrowerContractAddress: borrowerContract.address,
        usdcAddress: usdc.address,
  ...
  describe("gasless transactions", async () => {
    ...
    describe("when the forwarder is trusted", async () => {
      beforeEach(async () => {
        ...
        ;({tranchedPool} = await deployFlexibleFundedTranchedPool({
          borrower,
          borrowerContractAddress: borrowerContract.address,
          usdcAddress: usdc.address,
          id: "secondPool",
  ...
  describe("pay", async () => {
    ...
    beforeEach(async () => {
      ...
      ;({creditLine, tranchedPool} = await deployFlexibleFundedTranchedPool({
        borrower,
        borrowerContractAddress: borrowerContract.address,
        usdcAddress: usdc.address,
        id: "TranchedPool",
  ...
  describe("payMultiple", async () => {
    ...
    beforeEach(async () => {
      ...
      const firstTrancheDeploy = await deployFlexibleFundedTranchedPool({
        borrower,
        borrowerContractAddress: borrowerContract.address,
        usdcAddress: usdc.address,
      ...
      const secondTrancheDeploy = await deployFlexibleFundedTranchedPool({
        borrower,
        borrowerContractAddress: borrowerContract.address,
        usdcAddress: usdc.address,
